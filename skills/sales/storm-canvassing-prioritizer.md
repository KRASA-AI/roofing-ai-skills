...
**Fastest path — minimum viable input:** give the **storm footprint** (item 1 — a hail/wind swath, a storm-map file path, or even just a city-level headline) and run. That is the only blocking input. Everything else is read from `config.yml` or defaulted: service-area envelope from `service_area.zip_codes[]` + `service_area.licensed_counties[]`, crew and caller capacity from `team.canvasser_roster[]` / `team.phone_canvasser_roster[]`, damage thresholds from `weather_rules.*`, and script tone from `canvassing.script_personality`. Property-intelligence depth auto-degrades to the best source available (down to manual map) and is flagged; priority criteria fall back to the standard 10+ yr hail / 15+ yr wind cutoffs; CRM overlap is used only if supplied. When only a city headline is available, the skill works from the approximate center with a **degraded-confidence tag** rather than stopping to ask. The skill asks **at most one** question — only when no roster and no capacity numbers exist to size the plan. Don't interrogate for territories, thresholds, or tone that config already holds.

The full input list, annotated by how each is resolved:

1. **Storm footprint** *(blocking — the one input the skill can't run without)* — Hail swath (start/end coordinates, peak size, date/time), wind swath (peak gust, date/time), or the storm-map report file path. If only a city-level headline is available, say so and the skill works from the approximate center with a degraded-confidence tag.
2. **Service-area constraints** *(from config)* — Drive-time radius or ZIP whitelist; read from `service_area.zip_codes[]` and licensed-county filter from `service_area.licensed_counties[]`. Only ask if config is empty.
3. **Crew and caller capacity** *(from config)* — Read from `team.canvasser_roster[]` / `team.phone_canvasser_roster[]`. Only ask for door-knocker + phone-canvasser counts, shifts/day, daylight-only constraint, and HOA-restricted subdivisions when the roster is absent.
4. **Property intelligence access** *(inferred / optional)* — Which platforms the shop can call (Eagleview Horizon / One API, Nearmap, CAPE Analytics, county assessor, plain public search, or manual map). Auto-degrades to the best available source and flags the depth; never a blocker.
5. **Priority criteria** *(inferred / optional)* — Target roof-age cutoff, size bracket, material preference, carrier-based targeting. Defaults to 10+ yr (hail) / 15+ yr (wind) and `weather_rules.*` thresholds when unstated.
6. **Existing CRM overlap** *(optional)* — Addresses already in the CRM (previous customers, prior estimates, active leads) so canvassing can skip or differently-script them. Used when supplied; omitted cleanly when not.
...
**Efficiency notes:**
- Runs from defaults: the storm footprint is the only blocking input. Service area, roster, thresholds, and script tone all come from config; property-intelligence depth auto-degrades to the best available source and is flagged. Ask **one** question only when no roster and no capacity numbers exist to size the plan.
- Accept a city-level headline and proceed from the approximate center with a degraded-confidence tag rather than blocking on precise swath coordinates — a same-morning plan at 80% confidence beats a perfect plan after the competitor's trucks arrive.
- Cross-reference sibling skills: `predictive-lead-scorer` (score the resulting address list), `lead-response-automator` (when knocks and calls generate inbound), `roof-inspection-report` (the on-roof deliverable once a door converts).
